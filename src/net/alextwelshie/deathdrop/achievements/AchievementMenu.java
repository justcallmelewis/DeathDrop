...
public class AchievementMenu {
	...
	public Inventory getInventory(Player player) {
		Inventory inventory = Bukkit.createInventory(null, 27, "§8[AM] Home");
		{
			...
			if (PlayerManager.getInstance().hasAchievement(player,
					Achievement.FIRSTJOIN)) {
				item = new ItemStack(Material.INK_SACK, 1, (short) 10);
				lore.add("§eDescription: §b"
						+ AchievementAPI.getInstance().getDescription(
								Achievement.FIRSTJOIN));
			...
			meta.setDisplayName("§eName: §b"
					+ AchievementAPI.getInstance().getDisplayName(
							Achievement.FIRSTJOIN));
			meta.setLore(lore);
		...
		}
		{
			...
			ArrayList<String> lore = new ArrayList<>();
			if (PlayerManager.getInstance().hasAchievement(player,
					Achievement.FIRST_LAND_SUCCESS)) {
				item = new ItemStack(Material.INK_SACK, 1, (short) 10);
				lore.add("§eDescription: §b"
						+ AchievementAPI.getInstance().getDescription(
								Achievement.FIRST_LAND_SUCCESS));
