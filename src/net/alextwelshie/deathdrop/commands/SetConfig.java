        ...
        if (sender instanceof Player) {
            ...
            if (RankHandler.getInstance().isStaff(player)) {
                switch (args.length) {
                    case 0:
                    case 1:
                        player.sendMessage(Main.getPlugin().prefix + "§cIncorrect syntax. Please use: /setconfig [variable] [value]");
                        break;
                    case 2:
                        ...
                        if (variable.equalsIgnoreCase("gametype")) {
                            if (Main.getPlugin().getState() == GameState.INGAME) {
                                player.sendMessage(Main.getPlugin().prefix + "§cSorry, but you can't change the gametype in-game.");
                            } else {
                                if (value.equals("Enhanced")) {
                                    Main.getPlugin().config.set(variable, value);
                                    Main.getPlugin().setType(GameType.valueOf(value.toString()));

                                    player.sendMessage(Main.getPlugin().prefix + "§eValue of " + variable + " set to: " + value);
                           ...
                           if (Main.getPlugin().getState() == GameState.INGAME) {
                                player.sendMessage(Main.getPlugin().prefix + "§cSorry, but you can't change the max rounds in-game.");
                            } else {
                               if(value.toString().matches("[0-9]+")) {
                                   ...
                                   if(Main.getPlugin().maxRounds <= Main.getPlugin().round) {
                                       ...
                                       player.sendMessage(Main.getPlugin().prefix + "§eDue to this change, max rounds is now less than current round.");
                                       player.sendMessage(Main.getPlugin().prefix + "§eEnding game..");
                                   } 
                                   player.sendMessage(Main.getPlugin().prefix + "§eValue of " + variable + " set to: " + value);
                               } else {
                           ...
                           if (Main.getPlugin().getState() == GameState.INGAME) {
                                player.sendMessage(Main.getPlugin().prefix + "§cSorry, but you can't change the players needed to start in-game.");
                            } else {
                               if(value.toString().matches("[0-9]+")) {
                                   Main.getPlugin().config.set(variable, value);
                                   ...
                                   if(Bukkit.getOnlinePlayers().size() >= Main.getPlugin().neededToStart) {
                                       ...
                                       player.sendMessage(Main.getPlugin().prefix + "§eDue to this change, players needed is now less than online player count.");
                                       player.sendMessage(Main.getPlugin().prefix + "§eBeginning game..");
                                   }
                               } else {
                                   player.sendMessage(Main.getPlugin().prefix + "§cValue is not numeric.");
                               }
                           ...
                           if (Main.getPlugin().getState() == GameState.INGAME) {
                                player.sendMessage(Main.getPlugin().prefix + "§cSorry, but you can't change the max players in-game.");
                            } else {
                               if(value.toString().matches("[0-9]+")) {
                                   Main.getPlugin().config.set(variable, value);
