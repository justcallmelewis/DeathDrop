					...
					if (!Bukkit.getScheduler().isCurrentlyRunning(
						...
						if (!Main.getPlugin().ended) {
							...
							if (Main.getPlugin().round == (Main.getPlugin().maxRounds + 1)) {
								...
								for (Player all : Bukkit.getOnlinePlayers()) {
									Score score = Main.getPlugin().board
											.getObjective("scoreboard")
								...
								if (winners.size() >= 2) {
									String[] winnerName = new String[winners
									                                 .size()];
									...
									Bukkit.broadcastMessage(Main.getPlugin().prefix
											+ " ");
									Bukkit.broadcastMessage(Main.getPlugin().prefix
											+ "§3All players were tied at §d"
											+ highest + " §3points.");
								} else {
									Player winner = Bukkit.getPlayer(winners
											.get(0));
									Bukkit.broadcastMessage(Main.getPlugin().prefix
											+ "§e§lWINNER: §a"
											+ winner.getName());
								...
								Main.getPlugin().board.getObjective(
